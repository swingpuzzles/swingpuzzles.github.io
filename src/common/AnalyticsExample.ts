...
export function initializeWithGoogleAnalytics(measurementId: string) {
...
export function switchToClarity(projectId: string) {
    analyticsInitializer.switchToClarity(projectId);
    console.log('Switched to Microsoft Clarity');
}

export function switchToGoogleAnalytics(measurementId: string) {
    analyticsInitializer.switchToGoogleAnalytics(measurementId);
...
export function trackGameEvents() {
    // Game events
    analyticsManager.startGameSession(MainMode.Initial, SubMode.Normal, 'animals', 100);
