...
class PhysicsAggregateBuilder {
    attachGroundAggregate(ground: Mesh) {
    ...
    attachDragPolygonAggregate(mesh : Mesh) {
        mesh.physicsAggregate = new PhysicsAggregate(
            mesh,
            PhysicsShapeType.BOX,
            { mass: 0.1, friction: 0.9, restitution: 0.01 },
            ctx.scene
        );
    }

    attachPuzzlePieceAggregate(mesh : Mesh) {
        mesh.physicsAggregate = new PhysicsAggregate(
            mesh,
            PhysicsShapeType.BOX,
    ...
    attachInitialPuzzlePieceAggregate(mesh : Mesh) {
        mesh.physicsAggregate = new PhysicsAggregate(
            mesh,
            PhysicsShapeType.BOX,
